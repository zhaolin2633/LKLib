package cn.app.library.widget.expandabletextview;
...
public class ShrinkTextView extends TextView {
    ...
    private static final String TAG = ShrinkTextView.class.getSimpleName();
    ...
    public ShrinkTextView(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        TypedArray a = context.obtainStyledAttributes(attrs, R.styleable.ShrinkTextView);
        mFoldText = a.getString(R.styleable.ShrinkTextView_foldText);
        if (null == mFoldText) {
            mFoldText = DEFAULT_FOLD_TEXT;
        }
        mUnFoldText = a.getString(R.styleable.ShrinkTextView_unFoldText);
        if (null == mUnFoldText) {
            mUnFoldText = DEFAULT_UNFOLD_TEXT;
        }
        mFoldLine = a.getInt(R.styleable.ShrinkTextView_foldLine, DEFAULT_FOLD_LINE);
        if (mFoldLine < 1) {
            throw new RuntimeException("foldLine must not less than 1");
        }
        mTailColor = a.getColor(R.styleable.ShrinkTextView_tailTextColor, DEFAULT_TAIL_TEXT_COLOR);
        mCanFoldAgain = a.getBoolean(R.styleable.ShrinkTextView_canFoldAgain, DEFAULT_CAN_FOLD_AGAIN);
